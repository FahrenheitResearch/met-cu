...
def parity_context(sounding_profile):
...
def test_lfc_runtime_parity(sounding_profile):
    actual = metcu.lfc(sounding_profile["pressure"], sounding_profile["temperature"], sounding_profile["dewpoint"])
    expected = mpcalc.lfc(sounding_profile["pressure"], sounding_profile["temperature"], sounding_profile["dewpoint"])
    assert_runtime_close(actual, expected, 3e-2)


def test_el_runtime_parity(sounding_profile):
    actual = metcu.el(sounding_profile["pressure"], sounding_profile["temperature"], sounding_profile["dewpoint"])
    expected = mpcalc.el(sounding_profile["pressure"], sounding_profile["temperature"], sounding_profile["dewpoint"])
    assert_runtime_close(actual, expected, 3e-2)


def test_cape_cin_runtime_parity(sounding_profile):
    parcel_profile = mpcalc.parcel_profile(
        sounding_profile["pressure"],
        sounding_profile["temperature"][0],
        sounding_profile["dewpoint"][0],
    )
    actual = metcu.cape_cin(
        sounding_profile["pressure"],
        sounding_profile["temperature"],
        sounding_profile["dewpoint"],
        parcel_profile,
    )
    expected = mpcalc.cape_cin(
        sounding_profile["pressure"],
        sounding_profile["temperature"],
        sounding_profile["dewpoint"],
        parcel_profile,
    )
    assert_runtime_close(actual, expected, 5e-3)


def test_downdraft_cape_runtime_parity(sounding_profile):
    actual = metcu.downdraft_cape(
        sounding_profile["pressure"],
        sounding_profile["temperature"],
        sounding_profile["dewpoint"],
    )
    expected = mpcalc.downdraft_cape(
        sounding_profile["pressure"],
        sounding_profile["temperature"],
        sounding_profile["dewpoint"],
    )
    assert_runtime_close(actual, expected, 5e-1)


def test_parcel_profile_with_lcl_runtime_parity(sounding_profile):
    actual = metcu.parcel_profile_with_lcl(
        sounding_profile["pressure"],
        sounding_profile["temperature"],
        sounding_profile["dewpoint"],
    )
...
def test_water_latent_heat_runtime_parity():
    cases = [
    ...
    for func_name, temperature, atol in cases:
        actual = getattr(metcu, func_name)(temperature)
